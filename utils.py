...
def load_motif_annotations(specie: str,
    ...
    logging.basicConfig(level = level, format = format, handlers = handlers)
    log = logging.getLogger('load_motif_ann')
    ...
    if fname is None:
        ...
        # print(f'\tReading annot file {fname}')
    ...
    log.info(f'\t\tMean Motif similarity: {df["MotifSimilarityQvalue"].mean():.2f}, stdev: {df["MotifSimilarityQvalue"].std():.2f}, motif_similarity_fdr: {motif_similarity_fdr}')
    log.info(f'\t\tMean Orthologous Identity: {df["OrthologousIdentity"].mean():.2f}, stdev: {df["OrthologousIdentity"].std():.2f}, orthologous_identity_threshold: {orthologous_identity_threshold}')
    # Direct annotation
...
def get_TF_list(
    ...
    """
    Extracts unique transcription factors (TFs) from motif enrichment tables.

    Parameters:
    ----------
    motif_enrichment_table : pd.DataFrame
        DataFrame containing motif enrichment information.
    annotation : List[str]
        List of columns to extract TFs from.
...
def get_cistrome_per_TF(motif_hits_dict,
    ...
    """
    return list(set(sum([motif_hits_dict[x] for x in motifs if x in motif_hits_dict], [])))
