...
class DEM(MotifEnrichmentResult):
    ...
    def run(self, dem_db: DEMDatabase):
        ...
        log.info(f'Number of foreground scores: {len(foreground_scores_arr.flatten())}')
        ...
        log.info(f'Mean adjpval: {result["Adjusted_pval"].mean()}, stdev: {result["Adjusted_pval"].std()}, threshold: {self.adjpval_thr}')
        log.info(f'Mean Log2FC: {result["Log2FC"].mean()}, std: {result["Log2FC"].std()}, threshold: {self.log2fc_thr}')
        ...
        log.info(f"Motifs passing p-value threshold: {len(result[result['Adjusted_pval'] <= self.adjpval_thr])}")
        log.info(f"Motifs passing logFC threshold: {len(result[result['Log2FC'] >= self.log2fc_thr])}")

        self.motif_enrichment = result
        log.info(f"Adding motif-to-TF annotation")
        ...
        motif_hits = {}
        log.info(f'Avg motif hit score: {np.mean(foreground_scores_arr)}, stdev: {np.std(foreground_scores_arr)}, threshold {self.motif_hit_thr}')
