...
class Tracker:
    ...
    def __call__(self, fn):
        def wrapper(*args, **kwargs):
            log.info(' '*(self.indent * self.level) + '-' + fn.__name__)
            self.level += 1
            out = fn(*args, **kwargs)
            self.level -= 1
            return out
...
class MotifEnrichmentResult:
    ...
    def add_motif_annotation(
        ...
        # log.info(f'\tAdding motif annotations')
        # log.info(f'Path to motif annotations: {self.path_to_motif_annotations}')
        try:
            ...
            # log.info(f'\tAnnotation DataFrame = {annot_df.head()}')
            ...
            # log.info(f'\tmotif_enrichment_w_annot = {motif_enrichment_w_annot.head()}')
        except Exception as e:
            log.error(f"Error in add_motif_annotation: {e}")
    ...
    def get_cistromes(self):
        ...
        logging.basicConfig(level = level, format = format, handlers = handlers)
        log = logging.getLogger('cisTarget')
        log.info(f"\tMotif enrichment size: {self.motif_enrichment.shape if self.motif_enrichment is not None else 0}")
        log.info(f"\tAnnotations being used: {self.annotation_to_use}")
        log.info(f"\tMotif hits (database): {len(self.motif_hits['Database']) if 'Database' in self.motif_hits else 0}")
        log.info(f"\tMotif hits (region set): {len(self.motif_hits['Region_set']) if 'Region_set' in self.motif_hits else 0}")
